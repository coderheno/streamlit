...
def python_programming_guide():
    st.title("Python Programming Guide")

    st.sidebar.title("Python Guide Navigation")
    ...
    if option == 'Python Fundamentals':
        ...
        **Python Basics: Identifiers, Basic Types** covers the fundamental building blocks of Python programming:
        ...
        st.header("Experiential Learning: Program-2")
...
# Function to generate attendance slip text
def generate_attendance_slip(attendance_type, date, teacher_code, class_name, time, subject_name, start_reg_no, absentees_list, timestamp):
    slip = f"""
...
def log_to_excel(data, file_name='attendance_log.xlsx'):
    ...
    st.success("Attendance data logged successfully!")

# Streamlit UI for attendance entry
def attendance_tab():
    ...
    time = st.selectbox("Class Time", [
        "7 to 8", "8 to 9", "9.45 to 10.45", "10.45 to 11.45", 
        "11.45 to 12.45", "13.45 to 14.45", "14.45 to 15.45", 
        "15.45 to 16.45", "Other time"
    ])
    ...
    if st.button("Generate Attendance Slip"):
        ...
        slip = generate_attendance_slip(attendance_type, date, teacher_code, class_name, time, subject_name, start_reg_no, absentees_list, timestamp)
        st.success("Attendance slip generated.")
        st.download_button("Download Slip", slip, file_name="attendance_slip.txt")
    ...
    # Button to view and download the consolidated attendance log
    if st.button("View Consolidated Attendance Log"):
        if os.path.exists('attendance_log.xlsx'):
            ...
            st.download_button(
                label="Download Consolidated Attendance Log",
                data=output,
...
def main():
    ...
    tab = st.sidebar.radio("Go to", [
        ...
        "Fundamentals of Programming - Material",
        "Attendance",
        "Useful Links"
    ])
    ...
    elif tab == "Fundamentals of Programming - Material":
